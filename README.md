...
Сначала собрать приложение:

```bash
npm run build
```

Затем загрузить статическую сборку:
...
- не содержит статического лимита `550`;
- загружает все найденные файлы, кроме `.DS_Store`;
...
Число в сообщении `Загружаю N файлов` — это фактическое количество файлов
в текущем каталоге сборки, а не заданное ограничение. Например, текущая сборка
`out/` содержит 550 файлов без `.DS_Store`.
