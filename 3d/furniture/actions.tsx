...
export function ModuleMenu({ children, entity }: ModuleMenuProps) {
  ...
  const handleCopy = useCallback(() => {
    ...
    const copiedSource = toModuleEntity(entity, entity.position, entity.normal);
    copiedSource.id = entity.id;
    copiedSource.position = entity.position.clone();
    copiedSource.normal = entity.normal.clone();
    copiedSource.lock = entity.lock.clone();
    copiedSource.handleColor = entity.handleColor.clone();
    copiedSource.color = entity.color.clone();
    copiedSource.size = entity.size.clone();
    copiedSource.snapPlanes = entity.snapPlanes.map((plane) => ({
      point: [...plane.point] as [number, number, number],
      normal: [...plane.normal] as [number, number, number],
    }));
    copiedSource.halfExtents = [...entity.halfExtents] as [number, number, number];

    store.currentRawModule = copiedSource;
  }, [entity]);
            ...
            >
              ...
              <div
                onClick={() => {
                  ...
                  if (
                    store.currentRawModule &&
                    'id' in store.currentRawModule &&
                    store.currentRawModule.id === entity.id
                  ) {
